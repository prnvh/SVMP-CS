  ...
  return (
    <>
      <PageHeader
        ...
        copy={`${session.customer} on ${session.provider}. ${session.timestamp}.`}
        action={<StatusBadge tone={statusTone(session.status)}>{session.status}</StatusBadge>}
      />
            ...
            {session.transcript.map((message, index) => (
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  <p className="text-sm font-semibold capitalize">{message.speaker}</p>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <div className="rounded-[8px] border border-line p-4">
                  ...
                  <p className="mt-2 text-sm text-ink/76">
                    {session.confidence === null ? "Not scored" : session.confidence.toFixed(2)}
                  </p>
                </div>
              </div>
          ...
          <Panel title="Audit details" eyebrow="Governance">
            <dl className="grid gap-3 text-sm">
              <div className="flex justify-between gap-4">
                ...
                <dd className="font-semibold">{session.safety === null ? "Not scored" : session.safety.toFixed(2)}</dd>
              </div>
              <div className="flex justify-between gap-4">
