...
def _build_client(
    *,
    ...
    app = FastAPI()
    app.include_router(build_webhook_router(runtime_database, settings=settings or _settings()))
    return TestClient(app), runtime_database


def _signed_meta_request(payload: Mapping[str, Any]) -> tuple[bytes, dict[str, str]]:
    """Return a JSON body and matching Meta test signature headers."""
    ...
    return body, {
        ...
        "X-SVMP-Provider": "meta",
        "X-Hub-Signature-256": signature,
    }


def _signed_twilio_headers(payload: Mapping[str, str], *, url: str = "http://testserver/webhook") -> dict[str, str]:
    """Return Twilio test headers for the given form payload."""

    signature_base = url + "".join(f"{key}{payload[key]}" for key in sorted(payload))
...
def test_webhook_post_normalizes_meta_payload() -> None:
    ...
    body, headers = _signed_meta_request(payload)
...
def test_webhook_post_normalizes_twilio_form_payload() -> None:
    ...
    response = client.post(
        "/webhook",
        headers=_signed_twilio_headers(payload),
        data=payload,
    )
...
def test_webhook_post_rejects_unsigned_meta_payload() -> None:
    """Meta POSTs must carry a valid X-Hub-Signature-256 header."""
    ...
    assert response.json() == {"detail": "missing Meta webhook signature"}


def test_webhook_post_rejects_normalized_payloads_unless_enabled() -> None:
    """Already-normalized payloads should not be public by default."""
    ...
    assert response.status_code == 403
    assert response.json() == {"detail": "normalized webhook payloads are disabled"}
